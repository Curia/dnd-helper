...
export const ConstantsContext = createContext<
  { monsters: Partial<Monster>[] | null } | undefined
>(undefined);
...
export const ConstantsProvider = ({
  ...
  const [getMonsters] = useGetMonstersLazyQuery();

  useEffect(() => {
    const localStorageMonsters = localStorage.getItem(
      LOCAL_STORAGE_KEYS.MONSTERS,
    );
      ...
      getMonsters({ variables: { limit: 1000 } }).then((response) => {
        if (response.data?.monsters) {
          localStorage.setItem(
            LOCAL_STORAGE_KEYS.MONSTERS,
            JSON.stringify(response.data?.monsters),
          );
