...
export const ArmorClass: React.FC<{
  armorClass: Maybe<Maybe<MonsterArmorClass>[]> | undefined;
}> = ({ armorClass }) => {
  ...
  const additionArmor = (
    <>
      (
      {armorClass.slice(1).map((additionalClass, index) => (
        <Text
          key={index}
        >{`${additionalClass?.value} with ${additionalClass?.spell?.name}`}</Text>
      ))}
      )
    </>
  );
  ...
  return (
    <Flex alignItems={'center'} gap={'4'}>
      <Text display={'flex'} alignItems={'center'} fontSize="2xl">
        <Icon as={PiShieldDuotone} mr={'2'} /> {baseArmor?.value}{' '}
        {additionArmor}
      </Text>
    </Flex>
  );
