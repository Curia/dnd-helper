...
export const MonsterAttributes = ({ monster }: { monster: Partial<Monster> }) => {
      ...
      {Object.entries(ATTRIBUTE_TYPES).map(([, attributeName], index) => {
        ...
        return (
          <Tag key={index} variant='solid' colorScheme='teal'>
            {attributeName.slice(0, 3).toUpperCase()} {`${attributeValue} (+${calculateAbilityModifier(Number(attributeValue))})`}
          </Tag>
